...
async def make_config(bot: Client, msg: Message):
    try:
        if config_path.exists():
            return await msg.reply_text(text="**You have already made a config babu first delete it then you'll able to make it config**\n\n Use /del_config", reply_to_message_id=msg.id)
        else:
            ...
            while True:
                ...
                if str(n.text).isnumeric():
                    ...
                    if not str(target.text).isnumeric():
                        ...
                        await msg.reply_text(text="⚠️ **Babu Pleae Send Valid Target Channel Link or Username !**", reply_to_message_id=target.id)
                        continue

                else:
                    await msg.reply_text(text="⚠️ **Babu Pleae Send Integer Number not String !**", reply_to_message_id=n.id)
                    continue
            ...
            for _ in range(int(n.text)):
                ...
                for acocunt in config['accounts']:
                    if acocunt['Session_String'] == session.text:
                        return await msg.reply_text(text=f"**{acocunt['OwnerName']} account already exist in config babu you can't add same account multiple times 🤡**\n\n Error !")
